...
class TestService:
    ...
    def init_model(self):
        ...
        model = model.eval()
        TestService.tokenizer_2b = tokenizer
        TestService.model_2b = model

        # 加载向量匹配模型
        TestService.embedder = SentenceModel(
        ...
        for name, _ in commentMap:
            qE = TestService.embedder.encode([name])
            TestService.embeddingNameList.append(name)
            TestService.embeddingList.append(qE)
